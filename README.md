...
│   ├── generate_ieee_paper_figures.py
├── results/
│   ├── calibration/
│   ├── primary/
│   ├── secondary/
...
* `generate_ieee_paper_figures.py`
  Generates the three manuscript figures from the verified summary values, including the geometry comparison, mechanism-and-recovery analysis, and primary heatmap.
...
For each completed profile, the repository contains:

- `data/merged/`: merged row-level simulation outputs;
- `tables/`: derived summary and comparison tables;
- `summaries/`: compact run summaries and profile-level notes.

The included profiles are calibration, primary, secondary, sensitivity,
ablation, and exact sequential. The exact files available under each profile
depend on which outputs were generated by the corresponding analysis stage.

Column definitions and interpretation notes are provided in
...
The included result files are archival outputs from the completed production
campaign. They are provided to support inspection of the reported summaries and
do not need to be regenerated before running a new simulation.
